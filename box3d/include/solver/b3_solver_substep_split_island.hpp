//
// Created by Sherman on 2024/12/16.
//
...
#include "b3_contact_solver_substep.hpp"

#include "collision/b3_contact.hpp"


struct b3SolverSubstepSplitIsland: b3Solver {

    int32 m_body_count;
    b3ContactManager* m_contact_manager;
    b3Vec3r m_gravity;

    b3SolverSubstepSplitIsland(b3World* world, b3BlockAllocator* block_allocator):b3Solver(world, block_allocator) {
    ...
    static b3Solver* create(b3World* world, b3BlockAllocator* block_allocator) {
        void* mem = block_allocator->allocate(sizeof(b3SolverSubstepSplitIsland));
        return new(mem) b3SolverSubstepSplitIsland(world, block_allocator);
    }

    void destroy() override {
        this->~b3SolverSubstepSplitIsland();
        m_block_allocator->free(this, sizeof(b3SolverSubstepSplitIsland));
    }
