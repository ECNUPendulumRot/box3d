...
#ifndef B3_PLANE_SPHERE_COLLISION_ALGORITHM_H
#define B3_PLANE_SPHERE_COLLISION_ALGORITHM_H
...
class b3PlaneSphereCollisionAlgorithm : public b3CollisionAlgorithm {

public:

    explicit b3PlaneSphereCollisionAlgorithm(b3Dispatcher* dispatcher);
    ...
    struct CreateFunc : public b3CollisionAlgorithmCreateFunc {
        b3CollisionAlgorithm* create_collision_algorithm(b3Dispatcher* dispatcher, const b3Fixture* fixtureA, const b3Fixture* fixtureB, int& allocate_size) override {
            allocate_size = sizeof(b3PlaneSphereCollisionAlgorithm);
            void* mem = dispatcher->allocate_collision_algorithm(allocate_size);
            return new (mem) b3PlaneSphereCollisionAlgorithm(dispatcher);
        }
    };
};
