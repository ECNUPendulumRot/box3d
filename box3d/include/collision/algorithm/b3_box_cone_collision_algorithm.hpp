...
#ifndef B3_BOX_CONE_COLLISION_ALGORITHM_H
#define B3_BOX_CONE_COLLISION_ALGORITHM_H
...
class b3BoxConeCollisionAlgorithm : public b3CollisionAlgorithm {
...
public:
    ...
    explicit b3BoxConeCollisionAlgorithm(b3Dispatcher* dispatcher, b3PenetrationDepthSolver* pdSolver, int numPerturbationIterations, int minimumPointsPerturbationThreshold);
    ...
    struct CreateFunc : public b3CollisionAlgorithmCreateFunc {
        ...
        b3CollisionAlgorithm* create_collision_algorithm(b3Dispatcher* dispatcher, const b3Fixture* fixtureA, const b3Fixture* fixtureB, int& allocate_size) override {
            allocate_size = sizeof(b3BoxConeCollisionAlgorithm);
            void* mem = dispatcher->allocate_collision_algorithm(allocate_size);
            return new (mem) b3BoxConeCollisionAlgorithm(dispatcher, m_pd_solver, m_numPerturbationIterations, m_minimumPointsPerturbationThreshold);
        }
    };
};
