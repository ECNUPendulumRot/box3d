...
#include "solver/b3_si_solver_lxj.hpp"
...
void b3SISolverLxj::init_velocity_constraints()
{
...
void b3SISolverLxj::solve_velocity_constraints(bool is_collision)
{
    for (int32 i = 0; i < m_contact_count; ++i) {
        b3ContactVelocityConstraint *vc = m_velocity_constraints + i;
...
void b3SISolverLxj::correct_penetration()
{
    for (int32 i = 0; i < m_contact_count; ++i) {
        b3ContactVelocityConstraint *vc = m_velocity_constraints + i;
...
void b3SISolverLxj::init_friction_constraints()
{
    for (int i = 0; i < m_contact_count; ++i) {
        b3ContactVelocityConstraint* vc = m_velocity_constraints + i;
        b3FrictionConstraint* fc = m_friction_constraints + i;

        fc->m_index_a = vc->m_index_a;
        fc->m_index_b = vc->m_index_b;
        fc->m_point_count = vc->m_point_count;
        fc->m_mass_a = vc->m_mass_a;
        fc->m_mass_b = vc->m_mass_b;
        fc->m_inv_mass_a = vc->m_inv_mass_a;
        fc->m_inv_mass_b = vc->m_inv_mass_b;
        fc->m_I_a = vc->m_I_a;
        fc->m_I_b = vc->m_I_b;
        fc->m_inv_I_a = vc->m_inv_I_a;
        fc->m_inv_I_b = vc->m_inv_I_b;
        fc->m_friction = vc->m_friction;
        fc->m_ra = vc->m_ra;
        fc->m_rb = vc->m_rb;
        ...
        for (int k = 0; k < vc->m_point_count; k++) {
            support_impulse += vc->m_points[k].m_normal_collision_impulse;
            not_apply_support_impulse += vc->m_points[k].m_normal_contact_impulse;
            fc->m_ras[k] = vc->m_points[k].m_ra;
            fc->m_rbs[k] = vc->m_points[k].m_rb;
        }

        if (not_apply_support_impulse == 0) {
            b3Body* a = m_bodies[vc->m_index_a];
            b3Body* b = m_bodies[vc->m_index_b];
            real normal_force = (b->get_gravity() + b->get_force() - a->get_gravity() - a->get_force()).dot(vc->m_normal);
            not_apply_support_impulse += normal_force * m_timestep->m_dt;
        }

        support_impulse += not_apply_support_impulse;

        fc->m_not_applied_support_impulse = not_apply_support_impulse;
        fc->m_max_friction_impulse = fc->m_friction * support_impulse;
        fc->m_friction_impulse[0] = 0;
        fc->m_friction_impulse[1] = 0;
        b3_get_two_tangent_bases(fc->m_normal, fc->m_friction_axis[0], fc->m_friction_axis[1]);
    }
}


void b3SISolverLxj::solve_friction_constraints()
{
    for (int i = 0; i < m_contact_count; i++) {
        b3FrictionConstraint* fc = m_friction_constraints + i;

        b3Vector3r v_a = m_velocities[fc->m_index_a].linear();
        b3Vector3r w_a = m_velocities[fc->m_index_a].angular();
        b3Vector3r v_b = m_velocities[fc->m_index_b].linear();
        b3Vector3r w_b = m_velocities[fc->m_index_b].angular();

        b3Vector3r v_rel = v_b + w_b.cross(fc->m_rb) - v_a - w_a.cross(fc->m_ra);
        b3Vector3r v_rel_t = v_rel - fc->m_normal * v_rel.dot(fc->m_normal);
        ...
        b3Vector3r tangent = v_rel_t / v_rel_t_size;

        real v_rel_to_zero = v_rel_t_size * fc->m_mass_b;
        if (fc->m_mass_b == 0) {
            v_rel_to_zero = v_rel_t_size * fc->m_mass_a;
        }

        b3Vector3r friction_impulse = -v_rel_to_zero * tangent;
        b3Vector3r target_friction_impulse;
        for (int k = 0; k < 2; ++k) {
            real temp = fc->m_friction_impulse[k] + friction_impulse.dot(fc->m_friction_axis[k]);
            temp = b3_clamp(temp, -fc->m_max_friction_impulse, fc->m_max_friction_impulse);
            real value = temp - fc->m_friction_impulse[k];
            fc->m_friction_impulse[k] = temp;
            target_friction_impulse += value * fc->m_friction_axis[k];
        }

        // get the friction impulse, and we apply it to body A and body B.
        v_b += fc->m_inv_mass_b * target_friction_impulse;
        v_a += fc->m_inv_mass_a * (-target_friction_impulse);

        w_a += fc->m_inv_I_a * fc->m_ra.cross(-target_friction_impulse);
        w_b += fc->m_inv_I_b * fc->m_rb.cross(target_friction_impulse);

        // The support torque can make the angular velocity to zero ?
        // TODO:
        // 0 = w + I_inv(r x N), N = f * normal
        // f(r x normal) = -Iw, find max r and min f
        // if r x normal = -Iw, then r = n x (-Iw)
        // For body B
        // now only consider to balance the friction torque.
...
//void b3SISolverLxj::solve_friction_help(
...
void b3SISolverLxj::init(b3BlockAllocator *block_allocator, b3Island *island, b3TimeStep *step)
{

    b3Solver::init(block_allocator, island, step);

    void* memory = m_block_allocator->allocate(sizeof(b3FrictionConstraint) * m_contact_count);
    m_friction_constraints = new (memory) b3FrictionConstraint[m_contact_count];
