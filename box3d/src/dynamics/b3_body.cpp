...
b3Body::b3Body(const b3BodyDef &body_def): m_volume(0.0)
{
    ...
    const auto& p = body_def.m_init_p;
    const auto& q = body_def.m_init_q;
    const auto& v = body_def.m_init_v;
    const auto& w = body_def.m_init_w;

    const b3Matrix3r& R = q.rotation_matrix();
    m_affine_q << p.x(), p.y(), p.z(),
...
void b3Body::reset_mass_data()
{
    ...
    b3Matrix3r inertia = b3Matrix3r::zero();
    ...
    if (inertia.determinant() > 0) {
        ...
        inertia -= m_mass * bias_center;

        b3_assert(inertia.determinant() > 0.0);

        const real& xx = inertia.m_11;
        const real& yy = inertia.m_22;
        const real& zz = inertia.m_33;

        const real& a_xx = (yy + zz - xx) / real(2.0);
        const real& a_yy = (zz + xx - yy) / real(2.0);
        const real& a_zz = (xx + yy - zz) / real(2.0);
        const real& a_xy = -inertia.m_12;
        const real& a_xz = -inertia.m_13;
        const real& a_yz = -inertia.m_23;
