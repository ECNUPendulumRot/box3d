...
void box3d::b3SphereShape::get_view_data(b3ViewData* view_data) const {
    ...
    // Sample k_segments - 1 rings on the sphere and two points on the end of sphere
    // Sample 2 * k_segments points on the ring
    ...
    for(int i = 0; i < k_segments - 2; ++i) {
        ...
        view_data->m_F.row(index++) << second_ring_index - 1, 
                                       second_ring_index, 
                                       second_ring_index + point_number - 1;
        // two near ring
