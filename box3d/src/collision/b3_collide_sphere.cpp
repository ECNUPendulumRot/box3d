...
static void find_feature(const b3Vec3r &point, const b3CubeShape *cube, b3SphereCubeFeature &feature)
{
...
static void find_deep_penetration(
    const b3Vec3r& sphere_center,
    b3Manifold* manifold,
    const b3Vec3r& cube_h_xyz,
    const b3Transformr& cube_xf)
{
    ...
    b3Vec3r penetration = sphere_center.abs() - cube_h_xyz;
    // penetration < 0
    if(penetration.x > penetration.y) {
        // max penetration axis is x.
        if(penetration.x > penetration.z) {
            if(sphere_center.x > 0) {
                manifold->m_local_normal = cube_xf.rotation_matrix() * b3Vec3r(1.0, 0, 0);
            } else {
                manifold->m_local_normal = cube_xf.rotation_matrix() * b3Vec3r(-1.0, 0, 0);
            }
            ...
            b3Vec3r local_point(local_point_x, sphere_center.y, sphere_center.z);
        ...
        if (penetration.y > penetration.z) {
            if (sphere_center.y > 0) {
                manifold->m_local_normal = cube_xf.rotation_matrix() * b3Vec3r(0, 1.0, 0);
            } else {
                manifold->m_local_normal = cube_xf.rotation_matrix() * b3Vec3r(0, -1.0, 0);
            }
            ...
            b3Vec3r local_point(sphere_center.x, local_point_y, sphere_center.z);
            manifold->m_points[0].m_local_point = cube_xf.transform(local_point);
            manifold->m_points[0].id.key = 0;
    ...
    if (sphere_center.z > 0) {
  	    manifold->m_local_normal = cube_xf.rotation_matrix() * b3Vec3r(0, 0, 1.0);
    } else {
  	    manifold->m_local_normal = cube_xf.rotation_matrix() * b3Vec3r(0, 0, -1.0);
    }
    ...
    b3Vec3r local_point(sphere_center.x, sphere_center.y, local_point_z);
    manifold->m_points[0].m_local_point = cube_xf.transform(local_point);
    manifold->m_points[0].id.key = 0;
}
...
static void face_separation(
    b3Manifold* manifold,
    const b3Vec3r& axis,
    const b3Vec3r& sphere_center_local,
    real total_radius,
    ...
    const b3Transformr& cube_xf)
{
    ...
    if(separation < total_radius) {
        ...
        b3Vec3r local_point = sphere_center_local - sphere_center_local.array_dot(axis)
    ...
    const b3Transformr& cube_xf)
{
    ...
    b3Vec3r mid_point = cube->m_h_xyz.array_dot(axis);
    int zero_axis = -1;
    for (int i = 0; i < 3; ++i) {
        if (axis[i] == 0) {
            zero_axis = i;
    ...
    b3Vec3r point_a = mid_point;
    b3Vec3r point_b = mid_point;
    point_a[zero_axis] -= cube->m_h_xyz[zero_axis];
    point_b[zero_axis] += cube->m_h_xyz[zero_axis];

    b3Vec3r edge_normal = (point_b - point_a).normalized();
    b3Vec3r project_point = point_a + (sphere_center_local - point_a).dot(edge_normal) *
                                       edge_normal;
    ...
    if (separation < total_radius) {
        ...
        b3Vec3r local_normal = (sphere_center_local - project_point).normalized();
        manifold->m_local_normal = cube_xf.rotation_matrix() * local_normal;
        manifold->m_points[0].id.key = 0;

        // manifold->m_points[0].id.key = 0;

        b3Vec3r local_point = project_point + manifold->m_penetration * local_normal;
        manifold->m_points[0].m_local_point = cube_xf.transform(local_point);
    }
}
  ...
  const b3Transformr& cube_xf)
{
  ...
  b3Vec3r  point = cube->m_h_xyz.array_dot(axis);
  ...
  if(separation < total_radius) {
    ...
    b3Vec3r local_point = point + manifold->m_penetration * point.normalized();
    manifold->m_points[0].m_local_point = cube_xf.transform(local_point);
  }
}
