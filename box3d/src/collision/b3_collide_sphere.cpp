...
static void find_deep_penetration(const b3Vector3d &sphere_center, b3Manifold *manifold,
  ...
  if (penetration.x() > penetration.y()) {
	...
	if (penetration.x() > penetration.z()) {
	  ...
	  double local_point_x = sphere_center.x() +
	...
	if (penetration.y() > penetration.z()) {
	  ...
	  double local_point_y = sphere_center.y() +
  ...
  double local_point_z = sphere_center.z() +
	manifold->m_penetration * manifold->m_local_normal.z();
...
static void face_separation(b3Manifold *manifold, const b3Vector3d &axis,
							const b3Vector3d &sphere_center_local, double total_radius,
							const b3CubeShape *cube, const b3TransformD &cube_xf) {
  // axis maybe
  // (1, 0, 0)
  // (-1, 0, 0)
  // (0, 1, 0)
...
static void edge_separation(b3Manifold *manifold, const b3Vector3d &axis,
							const b3Vector3d &sphere_center_local, double total_radius,
							const b3CubeShape *cube, const b3TransformD &cube_xf) {
  ...
  b3Vector3d project_point = point_a + (sphere_center_local - point_a).dot(edge_normal) * 
	                         edge_normal;
...
static void vertex_separation(b3Manifold *manifold, const b3Vector3d &axis,
							  const b3Vector3d &sphere_center_local, double total_radius,
							  const b3CubeShape *cube, const b3TransformD &cube_xf) {
