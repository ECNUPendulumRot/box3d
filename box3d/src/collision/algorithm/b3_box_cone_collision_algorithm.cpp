...
b3BoxConeCollisionAlgorithm::b3BoxConeCollisionAlgorithm(b3Dispatcher* dispatcher, b3PenetrationDepthSolver* pdSolver, int numPerturbationIterations, int minimumPointsPerturbationThreshold)
    : b3CollisionAlgorithm(dispatcher), m_pd_solver(pdSolver), m_numPerturbationIterations(numPerturbationIterations),
    m_minimumPointsPerturbationThreshold(minimumPointsPerturbationThreshold)
{
}

b3BoxConeCollisionAlgorithm::CreateFunc::CreateFunc(b3PenetrationDepthSolver *pdSolver) {
    m_numPerturbationIterations = 0;
    m_minimumPointsPerturbationThreshold = 3;
    m_pd_solver = pdSolver;
}

void b3BoxConeCollisionAlgorithm::process_collision(
    const b3Fixture *fixtureA, const b3Fixture *fixtureB,
    const b3DispatcherInfo &info, b3PersistentManifold* manifold)
{
    // fixtureA is box and fixtureB is cone
...
//    gjk_pair_detector.set_minkowskiA(shapeA);
//    gjk_pair_detector.set_minkowskiB(shapeB);
