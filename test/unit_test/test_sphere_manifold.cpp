...
bool line_AABB_intersect(const Eigen::Vector3d &origin, const Eigen::Vector3d &dir,
                         const b3Vec3r &min_corner, const b3Vec3r &max_corner) {
...
void interact_object(Eigen::Vector3d &origin, Eigen::Vector3d &point) {
  ...
  for (int i = 0; i < 2; ++i) {
	...
	const b3Vec3r min_a = aabb.min();
	const b3Vec3r max_a = aabb.max();
...
bool callback_mouse_down(igl::opengl::glfw::Viewer &viewer, int button, int modifier) {
  ...
  // 获取鼠标点击位置的深度值
  glReadPixels((int)x, (int)(viewer.core().viewport[3] - y), 1, 1, 
...
void init() {
  ...
  viewer.data().add_edges(Eigen::RowVector3d(0, 0, 0), 
...
void add_mesh(int color_index /* = 1 */, b3CubeShape &cube) {
  ...
  E3MapMatrixX<double, Eigen::RowMajor> vertices(view_data.m_V, 
												 view_data.m_vertex_count, 3);
  E3MapMatrixX<int, Eigen::RowMajor> faces(view_data.m_F, view_data.m_face_count, 3);
...
void print_vector3d(const b3Vec3r &v) {
...
bool pre_draw(igl::opengl::glfw::Viewer &viewer) {
  ...
  for (int i = 0; i < 2; ++i) {
	...
	if (manifold.m_point_count > 0) {
	  ...
	  b3Vec3r contact_point = manifold.m_points[0].m_local_point;
	  std::cout << "contact_point: ";
	  print_vector3d(contact_point);
	  b3Vec3r contact_normal = manifold.m_local_normal;
